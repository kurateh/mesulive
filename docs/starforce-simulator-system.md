...
세부 설정의 `흔적 복구` 섹션은 아래 조건을 만족할 때만 활성화됩니다.
...
스타캐치는 시스템 상시 적용으로 변경되어, 기본 확률표(`probTable`) 자체가 이미 스타캐치 반영값입니다.
...
파괴 발생 시 아래 조건을 모두 만족하면 흔적 복구를 시도합니다.

- 해당 성수에 흔적 복구 ON (`restoreRecord[현재성] === true`)
...
복구 리소스가 `0, 0`인 경우(예: 일부 레벨의 20~22성)는 흔적 복구를 적용하지 않고 일반 파괴 처리로 진행합니다.
...
2. `simulateStarforce.ts`
   - 파괴 시 흔적 복구 비용 계산 및 성수 복귀 처리

## 9) 구현상 주의사항 (코드 해석 포인트)

아래는 문서화 시 꼭 알고 있어야 하는 구현 디테일입니다.

1. `getProbTable`은 `probTable`의 복제본을 만들어 이벤트를 반영합니다(원본 배열은 수정하지 않음).
2. 스타캐치는 상시 적용이며, 별도 `*1.05` 재계산 없이 `probTable` 값을 그대로 사용합니다.
3. 유지 분기에서 성수 하락 처리가 없습니다(유지 시 성수 변화 없음).
4. 파괴 발생 시 흔적 복구 조건을 만족하면 복구 비용을 추가로 지불하고 파괴 직전 성수로 돌아갑니다(아니면 12성으로 하락).
5. 파괴방지 UI 토글은 15/16/17성만 제공됩니다.
6. 흔적 복구 UI 토글은 15~22성 버튼을 제공하고, 섹션 자체는 레벨/목표성 조건을 만족할 때만 활성화됩니다.
